...
dmverity = 1  # variable for build to be dmverity enabled t, set if it is
autosar = 1  # variable for build to be autosar enabled t, set if it is
v2x = 1  # variable for build to be v2x enabled t, set if it is
encrypted = 1  # variable for file system to be encrypted or not, set if it is
modem = 1  # Variable for modem is available or not, set when it is
connected = 1  # Variable for modem is connected to the internet, set when it is
...
class SmokeTests(unittest.TestCase):
    ...
    def test_Collect_Performance_Metrics(self):
        ...
        # TBA
        # global stid
        global dmverity  # variable for build to be dmverity enabled , set if it is
        global autosar  # variable for build to be autosar enabled , set if it is
        global v2x  # variable for build to be v2x enabled , set if it is
        global encrypted  # variable for file system to be encrypted or not, set if it is
        global original_stderr 
        global modem  # Variable for modem is available or not, set when it is
        global connected  # Variable for modem is connected to the internet, set when it is
        tid = 'REFP_000'
        ssh = self.ssh  # handle
        ...
        # Adding code to check the /autonet/etc/build_vars file and make a decision about the build variant and use these variables to run test cases conditionally
        # Check if DM verity is enabled
        output = run(self, 'cat /autonet/etc/build_vars|grep VERITY:', tid)
        if "yes" in output:
            dmverity = 1
            log_green('DM VERITY    :Enabled', tid)
        else:
            dmverity = 0
            log_green('DM VERITY    :Disabled', tid)
        
        # check if Autosar is enabled
        output = run(self, 'cat /autonet/etc/build_vars|grep AUTOSAR:', tid)
        if "yes" in output:
            autosar = 1
            log_green('AUTOSAR      :Enabled', tid)
        else:
            autosar = 0
            log_green('AUTOSAR      :Disabled', tid)

        # check if V2X is enabled
        output = run(self, 'cat /autonet/etc/build_vars|grep V2X:', tid)
        if "yes" in output:
            v2x = 1
            log_green('V2X          :Enabled', tid)
        else:
            v2x = 0
            log_green('V2X          :Disabled', tid)
 
        # check if proprietary, autonet or user partitions are encrypted or not
        output = run(self, 'cat /autonet/etc/build_vars|grep proprietary', tid)
        if "must_not_encrypt" in output:
            encrypted = 0
            log_green('proprietary  :Not Encrypted', tid)
        else:
            encrypted = 1
            log_green('proprietary  :Encrypted', tid) 

        output = run(self, 'cat /autonet/etc/build_vars|grep autonet', tid)
        if "must_not_encrypt" in output:
            encrypted = 0
            log_green('autonet      :Not Encrypted', tid)
        else:
            encrypted = 1
            log_green('autonet      :Encrypted', tid)

        output = run(self, 'cat /autonet/etc/build_vars|grep user', tid)
        if "must_not_encrypt" in output:
            encrypted = 0
            log_green('user         :Not Encrypted', tid)
        else:
            encrypted = 1
            log_green('user         :Encrypted', tid)
        
        if encrypted == 0:
            log_green('System       :Not Encrypted', tid)
        else:
            log_green('System       :Encrypted', tid)
        ...
        if '2 packets transmitted, 2 packets received' in output:
            log_green("Sierra Wireless Cellular Modem(172.90.0.1): Available", tid)
            modem = 1
        else:
            log_red("Sierra Wireless Cellular Modem(172.90.0.1): Not Available", tid)
            modem = 0            
        ...
        while True:
            try:
                # print('in test')
                stdin, stdout, stderr = ssh.exec_command('ping -c 1 trumain-primary-ext.telematicsgateway.net >/dev/null; echo $?')  # check if there is any error in issuing the command
                result = stderr.read().decode('UTF-8').replace("\n", "")
                if len(result) > 0 and max_ping_time < 10:
                    # log_red("error in executing the command encountered" + result + ": FAILED", tid)
                    max_ping_time += 1
                    wait(1)
                    pass
                    ...
                    wait(1)
                    stdin, stdout, stderr = ssh.exec_command('ping -c 1 trumain-primary-ext.telematicsgateway.net >/dev/null; echo $?')
                    output = stdout.read().decode('UTF-8').replace("\n", "")
                    break
            except Exception as e:
                log_red("Operation error:" + str(e) + ": FAILED", tid)
...
#         self.assertNotEqual(text_found, None)
    ...
    def test_REFP_022_Sierra_Wirless_eCall(self):
        ...
        if '2 packets transmitted, 2 packets received' in output:
            log_green("Verify that internet can be accessed over the cellular connection: PASSED", tid)
            wait(2)
            output = run(self, 'cell_shell "ecall --exe=trig --TEST +33141081040 2C3CDXCT8FH915638 VEHICLE_PASSENGER_M1 TYPE_GASOLINE 2" &', tid)
            if 'OK' in output:
...
#             print(diff)
    ...
    def test_REFP_031_IPSEC(self):
        ...
        if output == '0':
            ...
            log_red("Verify that the IPSEC tunnel is configured and running successfully", tid)
    ...
    def test_REFP_033_System_Size(self):
        ...
        # Collect the storage Available
        output = runr(self, 'df -h|grep /dev/root', tid)
        root = 'Memory Available: ' + output
        root = root[49:]
        root_size = float(root[:3])
        # print(root_size)
        
        # Collect the storage Available
        output = runr(self, 'df -h|grep /dev/mapper/user', tid)
        user = 'Memory Available: ' + output
        user = user[49:]
        user_size = float(user[:3])
        # print(user_size)
        
        # Collect the storage Available
        output = runr(self, 'df -h|grep /dev/mapper/autonet', tid)
        autonet = 'Memory Available: ' + output
        autonet = autonet[49:]
        autonet_size = float(autonet[:3])
        # print (autonet_size)
        
        # Collect the storage Available
        output = runr(self, 'df -h|grep /dev/mapper/proprietary', tid)
        prop = 'Memory Available: ' + output
        prop = prop[49:]
        prop_size = float(prop[:3])
        # print(prop_size)
        ...
        if total_system_size <= 128:
            log_green("Verify that total System Size(%.2f) is under 128M: PASSED" % total_system_size, tid)
        else:
            log_red("Verify that total System Size(%.2f) is under 128M: FAILED" % total_system_size, tid)
