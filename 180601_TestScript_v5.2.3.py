...
class SmokeTests(unittest.TestCase):
    ...
    def test_Collect_Performance_Metrics(self):
        ...
        if max_ping_time > 8:
            log_red("Unable to successfully contact the Backend: FAILED", tid)
        else:
            log_green('Time to successfully contact backend: %.2f' % backend_time + ' Seconds', tid)
    ...
    def test_REFP_002_Default_IP_DNS_DHCP(self):
        ...
        ''' Removing this code because bash is talen out and the command fails if used this way
        try:
            stdin, stdout, stderr = ssh.exec_command(
    ...
    def test_REFP_013_Event_Logger_Process(self):
        ...
        # print (msg_time)
        msg = "This is an Event Logger Test " + str(msg_time)
        # print(msg)
...
# using run function this point on
    ...
    def test_REFP_026_getfeature(self):
        ...
        ssh = self.ssh  # handle
        output = runr(self, 'getfeature PwrSched|grep "PwrSched" >/dev/null; echo $?', tid)
        if output == '0':
        ...
        output = runr(self, 'getfeature PwrCntrl|grep "PwrCntrl" >/dev/null; echo $?', tid)
        if output == '0':
        ...
        output = runr(self, 'getfeature GpsConf|grep "GpsConf">/dev/null; echo $?', tid)
        if output == '0':
        ...
        output = runr(self, 'getfeature EcallNumber |grep "EcallNumber" >/dev/null; echo $?', tid)
        if output == '0':
        ...
        output = runr(self, 'getfeature EcallDevice |grep "EcallDevice" >/dev/null; echo $?', tid)
        if output == '0':
...
#             print(diff)
    ...
    def test_REFP_028_Filesystem_Failover(self):
        ...
        if kernel == '1':
            ...
            output = runr(self, 'echo hello > /mnt/hello.sh >/dev/null; echo $?', tid)
            if output == '0':
                log_green("Verify that command: 'echo hello > /mnt/hello.sh' was successfully issued: PASSED", tid)
            else:
                log_red("Verify that command: 'echo hello > /mnt/hello.sh' was successfully issued: FAILED", tid)            
