...
#         cdriver = Chrome(chrome_options=opts)
    ...
    def test_REFP_017_file_system_structure_en(self):
        ...
        try:
            ...
            # actual output displayed for non-encypted file system
            '''
            lear@sr-test1:~/eclipse-workspace/ReferencePlatform$ ssh root@172.90.0.122
            root@einstein-049F041758(5.2.6):~$ mount
            /dev/dm-0 on / type ext4 (ro,noatime,data=ordered)
            devtmpfs on /dev type devtmpfs (rw,relatime,size=866580k,nr_inodes=216645,mode=755)
            sysfs on /sys type sysfs (rw,nosuid,nodev,noexec,relatime)
            proc on /proc type proc (rw,relatime)
            securityfs on /sys/kernel/security type securityfs (rw,nosuid,nodev,noexec,relatime)
            smackfs on /sys/fs/smackfs type smackfs (rw,nosuid,nodev,noexec,relatime)
